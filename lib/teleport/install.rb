module Teleport
  class Install
    ...
    def _ruby
      ...
      if ruby_version =~ /1.8.7/ && ruby_version !~ /Enterprise Edition/
        ...
        if fails?("which gem")
          ...
          run "wget http://production.cf.rubygems.org/rubygems/rubygems-#{RUBYGEMS}.tgz"
          run "tar xfpz rubygems-#{RUBYGEMS}.tgz"
          Dir.chdir("rubygems-#{RUBYGEMS}") do
            run "ruby setup.rb"
          end
    ...
    def _apt
      ...
      dirty ||= _rewrite("/etc/apt/sources.list.d/teleport.list") do |f|
        f.puts "# Generated by teleport"
        apt.each do |i|
          f.puts i.line
        end
